...
import { meetings, proposedSlots, participants, users } from "../db/schema.js";
...
meetingRoutes.get("/:shortId", async (c) => {
  ...
  const html = renderMeetingPage(meeting, slots, meetingParticipants, organizer);
  return c.html(html);
});
...
meetingRoutes.post("/:shortId/select", async (c) => {
  ...
  const confirmedTime = `${selectedSlot.startTime.toLocaleDateString("en-US", { weekday: "long", month: "long", day: "numeric" })} at ${selectedSlot.startTime.toLocaleTimeString("en-US", { hour: "numeric", minute: "2-digit" })}`;
...
function renderMeetingPage(
  ...
  organizer?: typeof users.$inferSelect | null,
): string {
  ...
  const slotCards = slots
    .map(
      ...
      >
        <div class="slot-date">${s.startTime.toLocaleDateString("en-US", { weekday: "long", month: "long", day: "numeric" })}</div>
        <div class="slot-time">${s.startTime.toLocaleTimeString("en-US", { hour: "numeric", minute: "2-digit" })} - ${s.endTime.toLocaleTimeString("en-US", { hour: "numeric", minute: "2-digit" })}</div>
...
<body>
  <div class="container">
    ...
    ${isConfirmed ? `<p>This meeting is confirmed for <strong>${meeting.confirmedStart?.toLocaleDateString("en-US", { weekday: "long", month: "long", day: "numeric" })} at ${meeting.confirmedStart?.toLocaleTimeString("en-US", { hour: "numeric", minute: "2-digit" })}</strong>.</p>` : ""}

    ${!isConfirmed && !isCancelled ? `<p style="margin-bottom: 1rem;">Pick a time that works for you:</p>` : ""}
